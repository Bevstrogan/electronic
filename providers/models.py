...
class Provider(models.Model):
    ...
    network_type = models.PositiveSmallIntegerField(
        choices=NetworkLevel, verbose_name="Выбор поставщика"
    )
    ...
    def save(self, *args, **kwargs):
        if self.network_type == 0:
            ...
            self.network_type == 1 and self.supplier and self.supplier.network_type == 0
        ):
            self.level = 1
        elif (
            self.network_type == 2
            and self.supplier
            and self.supplier.network_type in [0, 1]
        ):
            self.level = 2
